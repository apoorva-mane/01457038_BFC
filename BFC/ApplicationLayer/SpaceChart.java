...
public class SpaceChart extends ApplicationFrame{
	static ArrayList<String> list;
	static String title;
public SpaceChart(String paramString,ArrayList<String> l){
	super(paramString);
	list = l;
...
private static CategoryDataset createDataset(){
	...
	for(int i=0;i<list.size();i++){
		...
		localDefaultCategoryDataset.addValue(Long.parseLong(arr[1].trim()),arr[0],arr[0]);
	}
...
private static JFreeChart createChart(CategoryDataset paramCategoryDataset){
	JFreeChart localJFreeChart = ChartFactory.createBarChart(title,"File Name", "File Size (Bytes)", paramCategoryDataset, PlotOrientation.VERTICAL, true, true, false);
